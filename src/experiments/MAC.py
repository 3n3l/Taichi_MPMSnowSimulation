...
@ti.kernel
def pressure_boundary_condition():
    for j in range(ny+2):
        pt[0,j] = pt[1,j]
        pt[nx+1,j] = pt[nx,j]
    for i in range(nx+2):
...
def interpolate_velocity():
    for i,j in V:
        # NOTE: how the face velocities are accessed
...
gui = ti.GUI(f'Re = {un * lx / mu:4.0f} V_mag', res=(nx+1, ny+1))
step = 0

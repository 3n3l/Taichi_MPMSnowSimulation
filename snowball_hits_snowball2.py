...
def main():
    ...
    gui = ti.GUI("Snowball hits snowball2", res=512, background_color=0x0E1018)
    mpm = MPM(
        gui=gui,
        ...
        initial_positions=np.array([[0.06, 0.5], [0.94, 0.53]], dtype=np.float32),
        initial_velocities=np.array([[4, 0], [-4, 0]], dtype=np.float32),
        initial_radii=np.array([0.05, 0.05], dtype=np.float32),
    )
    mpm.run()
